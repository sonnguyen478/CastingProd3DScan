...
namespace Interop.Common.CVB.Util
{
    public class UtilImage
    {
        ...
        public unsafe static Cvb.Image.IMG ImageMirror(Interop.Common.CVB.cEnum.eMirror _mirror, Cvb.Image.IMG _imageObject)
        {
            int iWidth = Cvb.Image.ImageWidth( (Cvb.Image.IMG)_imageObject );
            int iHeight = Cvb.Image.ImageHeight( (Cvb.Image.IMG)_imageObject );

            IntPtr[] addrVPAT = new IntPtr[ Cvb.Image.ImageDimension( (Cvb.Image.IMG)_imageObject ) ];
            Cvb.Image.VPAEntry* pVPAT;
            IntPtr imageBaseAddress;

            Cvb.Image.IMG tmpImage = _imageObject;

            try
            {

                // get VPA and base address of plane d
                for ( int d = 0 ; d < addrVPAT.Length ; d++ )
                {
                    Cvb.Image.GetImageVPA( tmpImage, d, out imageBaseAddress, out addrVPAT[ d ] );
                }

                switch ( _mirror )
                {
                    //좌우
                    case Interop.Common.CVB.cEnum.eMirror.LeftRight:
                        {
                            // invert image X-VPAT
                            for ( int d = 0 ; d < addrVPAT.Length ; d++ )
                            {
                                // convert void* to tagVPAEntry*
                                pVPAT = (Cvb.Image.VPAEntry*)addrVPAT[ d ].ToPointer();

                                // invert X-VPAT of plane d
                                for ( int x = 0 ; x < iWidth / 2 ; x++ )
                                {
                                    System.IntPtr tmp = pVPAT[ x ].XEntry;// save x value
                                    pVPAT[ x ].XEntry = pVPAT[ iWidth - x - 1 ].XEntry; // and swap
                                    pVPAT[ iWidth - x - 1 ].XEntry = tmp;             // X entries
                                }
                            }
                        }
                        break;
                    ...
                    // 상하
                    case Interop.Common.CVB.cEnum.eMirror.TopBottom:
                        {
                            // invert image X-VPAT
                            for ( int d = 0 ; d < addrVPAT.Length ; d++ )
                            {
                                ...
                                pVPAT = (Cvb.Image.VPAEntry*)addrVPAT[ d ].ToPointer();
                                // invert VPAT of plane d
                                for ( int y = 0 ; y < iHeight / 2 ; y++ )
                                {
                                    System.IntPtr tmp = pVPAT[ y ].YEntry;             // save y value
                                    pVPAT[ y ].YEntry = pVPAT[ iHeight - y - 1 ].YEntry; // and swap
                                    pVPAT[ iHeight - y - 1 ].YEntry = tmp;             // Y entries
                                }
            ...
            {
                //  Cvb.Image.CreateDuplicateImage( _imageObject , out shImage );
