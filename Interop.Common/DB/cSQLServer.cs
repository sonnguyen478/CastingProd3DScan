...
namespace Interop.Common.DB
{
    ...
    public class cSQLServer
    {
        ...
        public DataTable fnGetRawData(string _sDate, string _eDate, string _mainTable, string _subTable, string _mipCode, string _judge, int _dataCnt)
        {
            DataTable makeData = new DataTable();
            string strSQL = string.Empty;
            try
            {
                strSQL += "SELECT DBO.FNCCONVERTDATE_FROMSTRING( MAIN.DATE_TIME) AS DATE_TIME\n";
                strSQL += "      ,MAIN.MIP_CODE\n";
                strSQL += "      ,MAIN.SUB_CODE\n";
                strSQL += "      ,MAIN.SN_NO\n";
                strSQL += "      ,MAIN.TOT_RESULT\n";
                for ( int idx = 0 ; idx < _dataCnt ; idx++ )
                {
                    strSQL += string.Format( "      ,MAIN.RESULT_{0} + '(' + CONVERT(NVARCHAR, (CONVERT(NUMERIC(3,2), SUB.RES_VAL_{0}) * 100)) + '/' +\n", ( idx + 1 ).ToString( "00" ) );
                    strSQL += string.Format( "                              CONVERT(NVARCHAR, (CONVERT(NUMERIC(3,2), SUB.SET_VAL_{0}) * 100)) + ')' AS RES{0}\n", ( idx + 1 ).ToString( "00" ) );
                }
                strSQL += string.Format( "FROM   {0} AS MAIN\n", _mainTable );
                strSQL += string.Format( "      ,{0} AS SUB\n", _subTable );
                strSQL += "WHERE  MAIN.DATE_TIME = SUB.DATE_TIME\n";
                strSQL += "AND    MAIN.MIP_CODE = SUB.MIP_CODE\n";
                strSQL += "AND    MAIN.SN_NO = SUB.SN_NO\n";
                strSQL += string.Format( "AND    MAIN.DATE_TIME BETWEEN '{0}' AND '{1}'\n", _sDate, _eDate );
                if ( _mipCode != "ALL" )
                {
                    strSQL += string.Format( "AND    MAIN.MIP_CODE = '{0}'\n", _mipCode );
                }
                if ( _judge != "ALL" )
                {
                    strSQL += string.Format( "AND    MAIN.TOT_RESULT = '{0}'\n", _judge );
                }
                strSQL += "ORDER  BY MAIN.DATE_TIME";
                makeData = GetSelectTable( strSQL );
            }
            catch ( Exception ex )
            {
                cLog.FileWrite_Str( "fnGetRawData Error : " + ex.Message.ToString(), cLog.eLogType.EXCEPTION );
                makeData = null;
            }
            return makeData;
        }
